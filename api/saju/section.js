...
// 2단계: /api/saju/analyze 가 준 설계(섹션 제목·angle)와 만세력 컨텍스트로 '한 섹션 본문'만 생성.
// 만세력 포인트 차감 없음(컨텍스트 재사용). 프론트가 섹션마다 병렬 호출 → 점진적 렌더.
import { readJson, sendJson } from "../_lib/http.js";
import { generateSection, generateFollowup } from "../_lib/analysis.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { productId = "saju-analysis", profile, partner, context, section, otherTitles = [] } = body;
    if (!profile || !profile.name) return sendJson(res, 400, { message: "프로필이 필요합니다." });
    if (!context || !section || !section.title) return sendJson(res, 400, { message: "설계(섹션) 정보가 필요합니다." });
