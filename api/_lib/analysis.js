// OpenAI 로 사주 해설 글 생성 (가맹점 자기 OPENAI_API_KEY 사용).
// 모델 기본값 gpt-5.4-mini (Responses API + reasoning). 가맹점이 site_config.ai_model 로 변경 가능.
...
export async function generateAnalysis({ productId, productName, prompt, profile, manse, summary, model }) {
  ...
  if (!apiKey) {
    const err = new Error("OpenAI 키가 설정되지 않았습니다 (OPENAI_API_KEY).");
    err.statusCode = 503;
    throw err;
  }
...
- 상품: ${productName}.`;
  ...
  const callOnce = async (extraNote = "") => {
    const response = await client.responses.create({
      ...
      reasoning: { effort: isFullReport ? "high" : "low" },
      instructions: instructions + extraNote,
      input,
      text: {
        format: {
          type: "json_schema",
...
- 한자·사주용어를 title/headline/angle에 노출하지 않는다. "${profile.name}"님 기준.`;
  ...
  const response = await client.responses.create({
    model: model || DEFAULT_MODEL,
    reasoning: { effort: "low" }, // 설계는 작은 출력 → low(~8s), 60s 타임아웃 안전 + 점진적 UX 첫 대기 단축
    instructions,
    input,
    text: {
      format: {
...
- headline/제목/점수/개운은 다시 쓰지 말고 body만. ${subject} 기준.`;
  ...
  const callOnce = async (extra = "") => {
    const response = await client.responses.create({
      ...
      input,
      text: { format: { type: "json_schema", name: "saju_section", strict: true, schema: SECTION_SCHEMA } },
    });
...
- answer 한 필드만 출력한다.`;
  ...
  const callOnce = async (note = "") => {
    const response = await client.responses.create({
      model: model || DEFAULT_MODEL,
      reasoning: { effort: "high" }, // 결제형 상담 — 깊고 풍부한 답변(60s 타임아웃 내)
      instructions: instructions + note,
      input,
      text: { format: { type: "json_schema", name: "saju_followup", strict: true, schema: FOLLOWUP_SCHEMA } },
    });
    const t = response.output_text;
    if (!t) throw new Error("AI 응답이 비어 있습니다.");
    return JSON.parse(t);
...
- 럭키 컬러/넘버/아이템과 음식 추천은 오행 균형(부족/과다)을 근거로 고른다.`;
  ...
  const response = await client.responses.create({
    ...
    instructions,
    input,
    text: { format: { type: "json_schema", name: "daily_fortune", strict: true, schema: DAILY_SCHEMA } },
  });
