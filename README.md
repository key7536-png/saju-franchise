...
**진짜 만세력 계산(중앙 API) + AI 해설(OpenAI) + 카카오 로그인 + 토스 결제 + 관리자 통계**가 들어 있고,
...
api/                               Vercel 서버리스 함수
  saju/analyze.js                  중앙 만세력 호출 → OpenAI 해설
  orders.js, payments/confirm.js   토스 결제 (Supabase 주문 저장)
...
1. 고객이 프로필 입력 → 결제(토스) → 승인된 주문만 분석 진행.
2. `/api/saju/analyze` 가 **대표 중앙 만세력 API**를 호출(가맹점 API키 → 포인트 차감) → 실제 사주 원국 계산.
3. 그 결과 + 가맹점이 어드민에서 편집한 프롬프트로 **OpenAI(gpt-5.4-mini)** 가 해설 생성.
4. 결과는 Supabase 보관함에 저장, 방문·매출은 관리자 통계로 집계.

> ⚠️ 만세력 1회 = 가맹점 포인트 차감, AI 1회 = 가맹점 OpenAI 비용. (무료 운세도 포인트 소모)
